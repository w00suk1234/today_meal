...
`assets/data/food_db_kr_sample.json`의 영양값은 포트폴리오/테스트용 샘플 값입니다. 디저트, 음료, 스테이크처럼 Vision AI 시연에 자주 쓰이는 항목도 포함되어 있지만, 실제 기록 전에는 사용자가 음식명과 섭취량을 확인하는 흐름을 유지합니다.
...
$env:AI_IMAGE_DETAIL="low"
$env:AI_DAILY_LIMIT_PER_CLIENT="20"
$env:OPENAI_API_KEY="your-openai-api-key"
```

`AI_MODEL`은 교체 가능합니다. 비워두면 서버 API는 저비용 vision 입력이 가능한 기본 모델로 `gpt-4o-mini`를 사용합니다. 1차 운영은 `gpt-4o-mini`, 정확도 비교는 `gpt-4o`, 비용 비교는 계정/API에서 이미지 입력 지원 여부를 확인한 뒤 `gpt-5-nano`로 테스트하는 흐름을 권장합니다.

`AI_IMAGE_DETAIL`은 이미지 분석 detail 옵션입니다. 기본값은 비용 우선의 `low`이고, 정확도 비교 시 `auto`, 필요한 경우에만 `high`로 바꿔 테스트합니다.
...
AI 분석은 사진 기반 음식 후보, 신뢰도, 설명, 예상 섭취량만 반환합니다. 칼로리와 탄수화물/단백질/지방은 AI가 만들지 않고, 앱 내부에서 음식 후보를 로컬 데이터와 확인한 뒤 사용자가 선택한 섭취량 기준으로 계산합니다.

비용 방어 전략:

- 사용자가 `AI 음식 분석 시작` 버튼을 눌렀을 때만 분석 요청
- Flutter에서 이미지를 리사이즈/압축한 뒤 전송
- 같은 이미지 SHA-256 해시가 이미 분석된 경우 클라이언트 캐시 결과 재사용
- 새로고침 후에도 같은 이미지 분석 결과는 24시간 localStorage 캐시로 재사용
- 서버는 `imageHash + model + detail` 기준의 짧은 메모리 캐시로 반복 호출 방어
- OpenAI 프롬프트에는 로컬 음식 DB 목록을 넣지 않고, 사진 기반 후보만 요청
- 로컬 음식 DB 매칭은 AI 응답 이후 앱 내부에서 결정적으로 처리
- 서버 응답 후보 최대 5개로 제한
- 서버에서 과도하게 큰 이미지 요청 거부
- 서버에서 클라이언트별 하루 분석 횟수 기본 20회 제한
- TODO: Vercel KV 또는 Supabase table 기반 장기 캐시
- TODO: Vercel KV 또는 Supabase 기반 정확한 rate limit
