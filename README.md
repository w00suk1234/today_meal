...
Flutter Web 개발 서버와 Vercel API Route 서버는 서로 다를 수 있습니다.

```text
Flutter Web: http://127.0.0.1:5173
AI API Server: http://localhost:3000
```

`api/analyze-food.ts`는 Flutter dev server 안에서 자동으로 실행되지 않습니다. 로컬에서 실제 원격 AI 분석을 확인하려면 Vercel dev 같은 별도 API 서버를 켜고, Flutter에는 그 서버 주소만 전달합니다.
...
- `AI_API_BASE_URL`이 `https://example.vercel.app` 같은 placeholder인지 확인
- API 서버가 실제로 켜져 있는지 확인
- `OPENAI_API_KEY`가 서버 환경변수에 있는지 확인
- 브라우저 콘솔에 CORS 에러가 있는지 확인
- `/api/analyze-food`가 404를 반환하는지 확인
- OpenAI 응답이 401, 400, 429인지 서버 로그에서 확인
- 설정한 `AI_MODEL`이 이미지 입력과 JSON 응답을 안정적으로 처리하는지 확인

디버깅 로그는 민감정보를 출력하지 않습니다. API Key 값과 base64 이미지 전문은 로그에 찍지 않고, 이미지 길이와 응답 preview 일부만 출력합니다.
